...
public partial class UserAuthority
{
    ...
    {
        using var cmd = new NpgsqlCommand("create table public.user_authority (user_data_id uuid not null ,authority_type_id uuid not null ,last_update timestamp(6) with time zone not null  ,primary key (user_id,authority_type_id) );", connection);
